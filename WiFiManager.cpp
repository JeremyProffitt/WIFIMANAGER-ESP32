...
void WiFiManager::handleRoot() {
  ...
  String page = FPSTR(HTTP_WFMGR_HEAD);
  page.replace("{v}", "Options");
  page += FPSTR(HTTP_WFMGR_SCRIPT);
  page += FPSTR(HTTP_WFMGR_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_WFMGR_HEAD_END);
  page += "<h1>";
  page += _apName;
  page += "</h1>";
  page += F("<h3>WiFiManager</h3>");
  page += FPSTR(HTTP_WFMGR_PORTAL_OPTIONS);
  page += FPSTR(HTTP_WFMGR_END);
...
void WiFiManager::handleWifi(boolean scan) {
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (_minimumQuality == -1 || _minimumQuality < quality) {
          String item = FPSTR(HTTP_WFMGR_ITEM);
          String rssiQ;
          rssiQ += quality;
  ...
  page += FPSTR(HTTP_WFMGR_FORM_START);
  char parLength[2];
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    String pitem = FPSTR(HTTP_WFMGR_FORM_PARAM);
  ...
  if (_sta_static_ip) {
    ...
    String item = FPSTR(HTTP_WFMGR_FORM_PARAM);
    item.replace("{i}", "ip");
    item.replace("{n}", "ip");
    ...
    item = FPSTR(HTTP_WFMGR_FORM_PARAM);
    item.replace("{i}", "gw");
    item.replace("{n}", "gw");
  ...
  page += FPSTR(HTTP_WFMGR_FORM_END);
  page += FPSTR(HTTP_WFMGR_SCAN_LINK);
...
void WiFiManager::handleNotFound() {
  ...
  message += server->uri();
  message += "\nMethod: ";
  message += ( server->method() == HTTP_WFMGR_GET ) ? "GET" : "POST";
  message += "\nArguments: ";
  message += server->args();
  message += "\n";
